...
intchf_tmpDelete(com2, pOut, GuiNum){
	helpStr := "-all  tmpフォルダのファイルを全て空に`n-gui  Guiの位置とサイズを保存するファイルをカラに`n-item  一時的に保存のアイテムを全てカラに`n-2gyo  一時的に保存の二行以上アイテムを全てカラに`n-hotkeyOnText  一時的に保存のテキスト上のキーを全て削除`n-hotkeyAllGround  一時的に保存の常駐キーを全て削除"
